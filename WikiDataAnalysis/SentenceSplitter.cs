...
namespace WikiDataAnalysis
{
    ...
    {
        public static class MethodsForSuffixArray
        {
        ...
        public enum ProbTypeEnum { CdL, CdM, CxLdM, CmMdSTDE, sqCdS, sqCxLdS, lnCdS, lnCxLdS, Sigmoid, Entropy, Entropy_L, Hank }
        public const string probTypeString =
                                    ...
                                    "probType == ProbTypeEnum.Hank? Math.Log(CalHank(s.Substring(i, l))) :                                                                                        \n";
        ...
        public List<string> Split(string s, int maxWordLength, Func<string,int,Func<string,int>, double> method/*s, BaseDataLength, Count()*/, bool verbose)
        {
            try
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    {
                        double probLog = method(s, BaseDataLength(), Count);//count,entropy
                        var v = dp[i] + probLog;
                        if (v > dp[i + l])
                        {
                            dp[i + l] = v;
        ...
        double[] hank_r = new[] { 1, 1, 0.05, 0.0001 };
        double CalHank(string s)
        {
            int n = BaseDataLength();
            switch (s.Length)
            {
                case 1: return hank_r[0] * Count(s) / n;
                case 2:
                    return
                        s.Aggregate(hank_r[0], (v, c) => v * Count(c.ToString()) / n) +
                        hank_r[1] * Count(s) / n;
                case 3:
                    return
                        s.Aggregate(hank_r[0], (v, c) => v * Count(c.ToString()) / n) +
                        hank_r[1] * Count(s.Substring(0, 1)) / n * Count(s.Substring(0, 2)) / Count(s.Substring(0, 1)) * Count(s.Substring(1, 2)) / Count(s.Substring(1, 1)) +
                        hank_r[2] * Count(s) / n;
                case 4:
                    return
                        s.Aggregate(hank_r[0], (v, c) => v * Count(c.ToString()) / n) +
                        hank_r[1] * Count(s.Substring(0, 1)) / n * Count(s.Substring(0, 2)) / Count(s.Substring(0, 1)) * Count(s.Substring(1, 2)) / Count(s.Substring(1, 1)) * Count(s.Substring(2, 2)) / Count(s.Substring(2, 1)) +
                        hank_r[2] * Count(s.Substring(0, 1)) / n * Count(s.Substring(0, 2)) / Count(s.Substring(0, 1)) * Count(s.Substring(0, 3)) / Count(s.Substring(0, 2)) * Count(s.Substring(1, 3)) / Count(s.Substring(1, 2)) +
                        hank_r[3] * Count(s) / n;
                default: return 0;
            }
        }
